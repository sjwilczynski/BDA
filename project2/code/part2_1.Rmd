...
}", file="./jags1_files/model12")

cat("model {
  for(i in 1:N){
    bin.score[i] ~ dbern(p[i])
    logit(p[i]) <- 
      b0 + b.rand[subject_id[i], 1] + b0_age*age[i] + 
      (b1 + b.rand[subject_id[i], 2] + b1_age*age[i]+b1_treatment*treatment[i]) * time[i]
  }
...
cat("model {
  for(i in 1:N){
    ...
    logit(p[i]) <- 
      ...
      (b1 + b.rand[subject_id[i], 2] + b1_sex * sex[i]+b1_treatment*treatment[i]) * time[i]
  }
...
cat("model {
  for(i in 1:N){
    ...
    logit(p[i]) <- 
      b0 + b.rand[subject_id[i], 1] + b0_age*age[i] + 
      (b1 + b.rand[subject_id[i], 2] + b1_age*age[i]) * time[i]
  }
  ...
  b.rand.int.means <- mean(b.rand[,1])
  b.rand.slp.means <- mean(b.rand[,2])
}", file="./jags1_files/model2")

cat("model {
  for(i in 1:N){
    bin.score[i] ~ dbern(p[i])
    logit(p[i]) <- 
      b0 + b.rand[subject_id[i], 1] + 
      (b1 + b.rand[subject_id[i], 2] + b1_treatment*treatment[i]) * time[i]
  }
...
```{r}
als.model12 <- test.model(iters, "./jags1_files/model12", monitor=c("b0", "b1", "b0_sex", "b1_sex", "b0_age", "b1_age", "b.rand.sigma", "b.rand.corr"))
summarise.model(als.model12, "./../model12.pdf", iters)

als.model23 <- test.model(iters, "./jags1_files/model23", monitor=c("b0", "b1", "b0_age", "b1_age", "b1_treatment", "b.rand.sigma", "b.rand.corr"), data=als.data, inits = als.inits)
summarise.model(als.model23, "./../model23.pdf", iters)

als.model13 <- test.model(iters, "./jags1_files/model13", monitor=c("b0", "b1", "b0_sex", "b1_sex", "b1_treatment", "b.rand.sigma", "b.rand.corr"))
summarise.model(als.model13, "./../model13.pdf", iters)

als.model1 <- test.model(iters, "./jags1_files/model1", monitor=c("b0", "b1", "b0_sex", "b1_sex", "b.rand.sigma", "b.rand.corr"))
summarise.model(als.model1, "./../model1.pdf", iters)

als.model2 <- test.model(iters, "./jags1_files/model2", monitor=c("b0", "b1", "b0_age", "b1_age", "b.rand.sigma", "b.rand.corr"))
summarise.model(als.model2, "./../model2.pdf", iters)

als.model3 <- test.model(iters, "./jags1_files/model3", monitor=c("b0", "b1", "b1_treatment", "b.rand.sigma", "b.rand.corr"), data=als.data, inits = als.inits)
summarise.model(als.model3, "./../model3.pdf", iters)
...
DIC
```{r}
models.jags <- list(als.model12, als.model23, als.model13, als.model1, als.model2, als.model3)
...
```
```{r}
x <- gelman.diag(models.mcmc[[6]])
```
```{r}
y<- lapply(models.jags, getDIC)
```
```{r}
a5 <- summary(as.mcmc(als.model13))
a5.quantiles
```
