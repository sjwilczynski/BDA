...
test.model <- function(iterations, model.file, monitor=c("b0", "b1", "b.rand.sigma", "b.rand.corr"), data=als.data, inits=als.inits) {
 als.model.jags <- run.jags(model = model.file, data = dump.format(data), monitor = monitor, n.chains = 3, inits = inits, burnin = iterations/3, sample = iterations, keep.jags.files = F, method = "parallel")
 als.model.jags
}
...
summarise.model <- function(model, plots.file, iterations) {
  ...
  superdiag(als.model.mcmc, burnin = iterations/3) 
}
```
